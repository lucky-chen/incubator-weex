...
public class WXSDKInstance implements IWXActivityStateListener, View.OnLayoutChangeListener {
    ...
    public void addLayerOverFlowListener(String ref) {
        if (mLayerOverFlowListeners == null) { mLayerOverFlowListeners = new ArrayList<>(); }
        mLayerOverFlowListeners.add(ref);
    }

    public void removeLayerOverFlowListener(String ref) {
        if (mLayerOverFlowListeners != null) { mLayerOverFlowListeners.remove(ref); }
    }
     ...
     * @param flag         RenderStrategy {@link WXRenderStrategy}
     */
    public void render(String pageName, String template, Map<String, Object> options, String jsonInitData,
            ...
            if (getUIContext() != null) {
                new AlertDialog.Builder(getUIContext())
                    ...
                    .setMessage("We highly recommend you to set pageName. Call" +
                        "\nWXSDKInstance#render(String pageName, String template, Map<String, Object> options, String"
                        + " jsonInitData, WXRenderStrategy flag)\n"
                        +
                        "to fix it.")
                    .show();
            }
    ...
    private void ensureRenderArchor() {
        if (mRenderContainer == null) {
            if (getContext() != null) {
                ...
                mRenderContainer.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
    ...
    private void renderInternal(String pageName,
        ...
        if (WXEnvironment.sDynamicMode && !TextUtils.isEmpty(WXEnvironment.sDynamicUrl) && renderOptions.get(
            "dynamicMode") == null) {
    ...
    private void renderByUrlInternal(String pageName,
        ...
        mApmForInstance.doInit();
        getWXPerformance().renderTimeRequestOrigin = WXUtils.getFixUnixTime();
     ...
     */
    @Deprecated
    public void render(String pageName, String template, Map<String, Object> options, String jsonInitData, int width,
                       int height, WXRenderStrategy flag) {
     ...
     */
    @Deprecated
    public void renderByUrl(String pageName, final String url, Map<String, Object> options, final String jsonInitData,
    ...
    public void onActivityPause() {
        ...
        if (!isCommit) {
            ...
            if (mUserTrackAdapter != null) {
                mUserTrackAdapter.commit(mContext, null, IWXUserTrackAdapter.LOAD, mWXPerformance,
                    getUserTrackParams());
            }
    ...
    public void onRenderSuccess(final int width, final int height) {
        ...
        if (mRenderListener != null && mContext != null) {
            ...
            if (mUserTrackAdapter != null) {
                ...
                mUserTrackAdapter.commit(mContext, null, IWXUserTrackAdapter.JS_BRIDGE, performance,
                    getUserTrackParams());
            }
    ...
    public void onChangeElement(WXComponent component, boolean isOutOfScreen) {
        ...
        if (isDestroy() || null == mRenderContainer || mWXPerformance == null || "videoplus".equals(
        ...
        if (mHasCreateFinish) {
            lazyLoadTime = lastElementChangeTime - mWXPerformance.renderTimeOrigin
                - mWXPerformance.callCreateFinishTime;
        ...
        if (component.mIsAddElementToTree) {
            ...
            if (!isOutOfScreen) { getWXPerformance().interactionViewAddLimitCount++; }
        ...
        if (!isOutOfScreen) {
            ...
            Log.w("jsLog", "[wxapm][interaction]"
                + ",componentType :" + component.getComponentType()
                + ",id :" + component.getRef()
                + ",curTime :" + (WXUtils.getFixUnixTime() - component.getInstance().getWXPerformance().renderTimeRequestOrigin)
                + ",componentBuildTime :" + component.createCostTime
                + ",pageName :"+component.getInstance().getWXPerformance().pageName
            );

            mApmForInstance.arriveInteraction(getWXPerformance().interactionViewAddLimitCount,
    ...
    public void onComponentCreate(WXComponent component, long createTime) {
        component.createCostTime = createTime;
    ...
    public void firstScreenRenderFinished() {
        ...
        if (!mHasSetCreateFinishFsTime) {
            mWXPerformance.fsRenderTime = System.currentTimeMillis();
            if (mHasCreateFinish) { mHasSetCreateFinishFsTime = true; }
        }
            ...
            if (layoutParams != null) {
                ...
                if (mRootComp != null && layoutParams != null) {
                    ...
                    WXBridgeManager.getInstance().post(new Runnable() {
                        @Override
                        public void run() {
                            WXBridgeManager.getInstance().setDefaultRootSize(getInstanceId(), realWidth, realHeight,
                                isWidthWrapContent,
                                isHeightWrapContent);
                        }
                    });
    ...
    public void fireEvent(String elementRef, final String type, final Map<String, Object> data,
                          final Map<String, Object>
                              domChanges, List<Object> eventArgs) {
        fireEvent(elementRef, type, data, domChanges, eventArgs, null);
    }

    public void fireEvent(String elementRef, final String type, final Map<String, Object> data,
                          final Map<String, Object> domChanges, List<Object> eventArgs, EventResult callback) {
    ...
    class WXHttpListener implements IWXHttpAdapter.OnHttpListener {
        ...
        private WXHttpListener(String pageName, Map<String, Object> options, String jsonInitData, WXRenderStrategy flag,
        ...
        public void onHttpFinish(WXResponse response) {
            ...
            if (null != response && response.extendParams != null) {
                ...
                if (isNet(mWXPerformance.requestType) && mUserTrackAdapter != null) {
                    ...
                    } else if ("200".equals(response.statusCode) && (response.originalData == null
                        || response.originalData.length <= 0)) {
                        performance.errCode = WXErrorCode.WX_ERR_JSBUNDLE_DOWNLOAD.getErrorCode();
                    ...
                    if (mUserTrackAdapter != null) {
                        mUserTrackAdapter.commit(getContext(), null, IWXUserTrackAdapter.JS_DOWNLOAD, performance,
                            null);
                    }
                }
            }
            ...
            } else if (response != null && response.originalData != null && TextUtils.equals("-206",
                response.statusCode)) {
    ...
    private void onInterceptInstanceEvent(String instanceId, String elementRef, String type, Map<String, Object> data,
